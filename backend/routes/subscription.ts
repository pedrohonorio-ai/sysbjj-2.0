  ...
  try {
    ...
    await prisma.systemLog.create({
      data: {
        ...
        details: `Exclusão DEFINITIVA de todas as dependências do Professor ${targetUserEmail} (ID: ${targetUserId}) homologada com sucesso.`,
        category: 'Admin_Audit',
        deviceInfo: req.headers['user-agent'] || 'Desconhecido',
      }
    });
    ...
    return res.json({
      success: true,
      message: `🥋 OSS! A conta do Professor ${targetUserEmail} e TODOS os alunos e dependências foram removidos por completo do Dojo.`
    });
