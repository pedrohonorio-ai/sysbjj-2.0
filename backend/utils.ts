...
export const handleApiError = (res: Response, error: any, collection: string) => {
    ...
    if (dbUrl === "INVALID_PLACEHOLDER_ERROR" || message.includes("invalid_host")) {
      ...
      res.status(status).json({
        ...
        details: "Vá em Settings > Secrets e insira a string de conexão real do seu Neon.",
    ...
    if (isConnectionError) {
      ...
      if (host.includes("neon.tech")) {
          ...
          troubleshooting.push("Verifique se o Neon não está em modo de repouso.");
      }
      res.status(status).json({
        error: `O Dojo Cloud (Banco de Dados) está inacessível${hostInfo}.`,
        details: "Verifique se o Host e a Senha estão corretos.",
        path: collection,
        troubleshooting
      });
      return;
