...
export const loginHandler = async (req: Request, res: Response) => {
  ...
  try {
    ...
    Promise.resolve().then(async () => {
      try { await prisma.user.update({ where: { id: user.id }, data: { lastLoginAt: new Date(), lastActivityAt: new Date(), active: true } }); } catch (_) {}
      try {
...
export const forgotPasswordHandler = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.json({ success: true, message: 'Link gerado.', token: resetToken });
  } catch (error: any) {
    res.status(500).json({ success: false, error: error.message });
  }
};
