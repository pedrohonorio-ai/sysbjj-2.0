...
const BusinessHub: React.FC = () => {
  ...
  const { products, plans, ledger, addProduct, addPlan, addExtraRevenue } = useData();
  const [activeTab, setActiveTab] = useState<'shop' | 'plans' | 'reports'>('shop');
    ...
    <div className="space-y-8 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl border border-slate-200 dark:border-white/5">
           <button 
             ...
             className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'shop' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-md' : 'text-slate-400'}`}
           >
             Loja
           </button>
           <button 
             onClick={() => setActiveTab('plans')}
             className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'plans' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-md' : 'text-slate-400'}`}
           >
             Planos
           </button>
           <button 
             onClick={() => setActiveTab('reports')}
             className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'reports' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-md' : 'text-slate-400'}`}
           >
             Relatórios
           </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-8 bg-blue-600 rounded-[2.5rem] text-white shadow-xl shadow-blue-600/20 relative overflow-hidden">
          ...
          <h2 className="text-3xl font-black italic">R$ {ledger.filter(e => e.type === 'ExtraRevenue').reduce((acc, e) => acc + e.amount, 0).toLocaleString()}</h2>
        ...
        <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white shadow-xl shadow-slate-900/20 relative overflow-hidden">
          <div className="absolute -right-4 -bottom-4 opacity-10"><Package size={120} /></div>
          <p className="text-[10px] font-black uppercase tracking-widest opacity-80 mb-1">Itens em Estoque</p>
          <h2 className="text-3xl font-black italic">{products.reduce((acc, p) => acc + (p.stock || 0), 0)} UND</h2>
          <div className="mt-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest bg-white/10 w-fit px-3 py-1 rounded-lg text-emerald-400">
             <ShieldCheck size={14} />
             Estoque Saudável
          </div>
        </div>
        <div className="p-8 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-xl relative overflow-hidden group">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Ticket Médio Extra</p>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white italic">R$ 145,00</h2>
          <button className="mt-4 flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 group-hover:gap-3 transition-all">
             Ver Analíticos <ArrowUpRight size={14} />
          </button>
        </div>
      </div>
                ...
                {filteredProducts.map(product => (
                  ...
                  >
                    <div className="w-full aspect-square bg-slate-50 dark:bg-white/5 rounded-3xl mb-6 flex items-center justify-center relative overflow-hidden">
                       ...
                       <div className="absolute top-4 right-4 px-3 py-1 bg-emerald-500 text-white text-[8px] font-black uppercase tracking-widest rounded-lg">
                         Em Estoque
                       </div>
                    </div>
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center justify-between pt-4">
                        ...
                        <button 
                          onClick={() => {
                            const studentName = prompt('Nome do aluno que comprou:');
                            if(studentName) {
                               addExtraRevenue({
