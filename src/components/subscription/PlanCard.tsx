...
const PlanCard: React.FC<PlanCardProps> = ({ subscription }) => {
  ...
  const formattedPlan = typeof safeSubscription.plan === 'string'
    ? safeSubscription.plan.replaceAll("_", " ").toUpperCase()
    : "FREE";

  const isNearLimit = (safeSubscription.usagePercent || 0) >= 80;
  const isAtLimit = (safeSubscription.usagePercent || 0) >= 100;
    ...
    <div className="bg-slate-900 border border-white/10 rounded-[2.5rem] p-8 text-white relative overflow-hidden group shadow-2xl">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div>
          ...
          <h3 className="text-2xl font-black italic uppercase tracking-tighter">🥋 Plano <span className="text-blue-500">{formattedPlan}</span></h3>
        </div>
        <div className="bg-white/5 border border-white/10 px-4 py-2 rounded-2xl">
          <p className="text-[8px] font-black uppercase tracking-widest text-slate-500">Mensalidade</p>
          <p className="text-lg font-black italic">
            {Number(safeSubscription.monthlyPrice || 0).toLocaleString("pt-BR", {
              style: "currency",
              currency: "BRL"
            })}
          </p>
      ...
      <div className="space-y-6 relative z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
             ...
             <div>
                ...
                <p className="text-xl font-black italic">{safeSubscription.currentStudents} <span className="text-slate-600">/ {safeSubscription.maxStudents === 999999 ? '∞' : safeSubscription.maxStudents}</span></p>
             </div>
          </div>
          <div className="text-right">
             <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Progresso</p>
             <p className={`text-xl font-black italic ${isAtLimit ? 'text-rose-500' : isNearLimit ? 'text-amber-500' : 'text-emerald-500'}`}>{Math.round(safeSubscription.usagePercent)}%</p>
          </div>
        </div>
        ...
        <div className="w-full bg-white/5 h-3 rounded-full overflow-hidden border border-white/5">
          <motion.div 
            initial={{ width: 0 }}
            animate={{ width: `${safeSubscription.usagePercent}%` }}
        ...
        {isAtLimit ? (
          ...
          <div className="flex items-center gap-3 p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-amber-500">
             <TrendingUp size={20} />
             <p className="text-[9px] font-black uppercase tracking-widest leading-relaxed">
               Atenção Sensei: Sua academia está crescendo rápido! Você está próximo do limite de {safeSubscription.maxStudents} alunos.
             </p>
          </div>
        ) : (
